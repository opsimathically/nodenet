...
try {
  ...
  const assertion =
    "const m=await import('@opsimathically/nodenetscanner'); if(typeof m.createScanner!=='function'||typeof m.inspectNetworkContext!=='function'||m.RESULT_BATCH_SCHEMA_VERSION!==1||m.SUPPORTED_SCAN_PROBES.length!==6)process.exit(1); const s=await m.createScanner(); await s.close()";
